...
module.exports.createCustomer=async(req,res,next)=>{
    try {
        ...
        var end= await  mongo.db.collection("Roomcounts").findOne({roomId:req.body.roomId});
        ...
        var data = await mongo.db.collection("customer").insertOne(req.body);
            console.log("yes");
       var a= await mongo.db.collection("Roomcounts").updateOne({roomId:req.body.roomId}, { $set: {bookingStatus:1} })
